...
function TableHeader({ className, ...props }: React.ComponentProps<"thead">) {
  return (
    <thead
      ...
      className={cn(
        "[&_tr]:border-b bg-muted/60 backdrop-blur-sm sticky top-0 z-10",
        className
      )}
      {...props}
    />
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-accent/45 data-[state=selected]:bg-accent border-b border-border/70 transition-colors duration-150",
        className
      )}
      {...props}
    />
